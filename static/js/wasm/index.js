...
window.AllFunc = {};
var AllFuncReceivedResolve;
var funcReceived = new Promise(resolve => {
...
function load(){
    if(!window.wasmIsLoad) {
        LoadWasm();
        if (!WebAssembly.polyfill) { // polyfill
            WebAssembly.polyfill = async (resp, importObject) => {
        ...
        let inst;
        console.log(WebAssembly.polyfill)
        WebAssembly.polyfill(fetch("/static/main.wasm"), go.importObject).then(async (result) => {
...
function creat_Key() {
    $('#creat_Key').on('click',function () {
        ...
        createKey(creData).then(res => {
            ...
            console.log(reqData);
            var url = "/wasmKey";
            echo.ajax.post(url,reqData,function (data) {
                echo.box.close();
